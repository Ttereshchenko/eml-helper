...
- [.agents/agents-routing.md](.agents/agents-routing.md) — which specialized agent to spin up and when (EML language specialist, mail conversion specialist, SMTP specialist, test author, security/audit reviewer). Re-read before delegating a focused sub-task to a sub-agent. For Claude Code the roles map to `.claude/agents/*.md`; for Antigravity use it as the Agent Manager delegation policy.
