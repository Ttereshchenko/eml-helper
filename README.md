...
- **Sending EML** — a swaks-equivalent embedded SMTP client. Right-click an `.eml` file in the editor or project view and choose **Send EML…** to push the file's bytes through a configured server. Highlights:
  ...
  - **Envelope address handling and Bcc safety** — the envelope `From` / `To` accept display-name forms (`Alice <alice@example.com>`) and comma-separated recipient lists (quoted commas and groups included); MailKit sends the bare address each server expects. A `Bcc:` header in the EML is stripped from the transmitted message so blind recipients are never disclosed, and the dialog warns which addresses it removed (they are not silently added as recipients)
