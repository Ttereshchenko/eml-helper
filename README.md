...
- **Compare two EML files** — Select two `.eml` files in the Project view and choose **Compare EML Files**, or right-click a single `.eml` (in the Project view or the open editor) and choose **Compare EML With…** to pick the other one. The two messages open in the IDE's diff viewer in a decoded, normalized form: RFC 2047 header words and base64 / quoted-printable bodies are decoded, headers are sorted into a canonical order, attachments are reduced to a name / type / size / SHA-256 manifest, and volatile noise (random MIME boundary tokens, transfer encoding, header order, line endings) is canonicalized — so only *meaningful* differences show instead of encoding artifacts. Forwarded or journaled messages (`message/rfc822`) are compared recursively, so the embedded email's own headers and body are decoded and diffed too. A toolbar toggle in the diff flips between this decoded view and the raw message source (so a raw byte-level comparison is one click away). In the editor toolbar, **Compare EML With…** sits under a single **MailKit** dropdown button (alongside **Send EML…**, stacked vertically); it can be hidden via **Settings > Editor > MailKit**.
- **MSG → EML conversion** — Right-click any Outlook `.msg` file in the Project view and choose **Convert to EML** to produce a standards-compliant `.eml` next to the source and open it in the editor. Handles HTML/text/RTF bodies, attachments (including OLE-embedded objects, exported as `.ole`), recursively-converted embedded `.msg` messages, and RFC 2047 encoding for non-ASCII headers. S/MIME messages keep their signed/encrypted envelope verifiable (`multipart/signed` / `application/pkcs7-mime`), calendar items and meeting requests export an `invite.ics` (with recurrence, time zone and all-day support), contacts a `contact.vcf` and tasks a `task.ics`. Outlook metadata survives: conversation threading (`Thread-Topic`/`Thread-Index`), `Importance`, `Sensitivity`, `Reply-To`, categories (`Keywords`) and read-receipt requests, and "sent on behalf of" messages keep the author in `From:` with the transmitter in `Sender:`. When a message has no original internet headers, MailKit synthesizes `From`/`To`/`Subject`/`Date` from MAPI properties and records which ones in an `X-MailKit-Synthesized-Headers` field.
- **PST / OST → EML conversion** — Right-click any Outlook `.pst` or `.ost` archive in the Project view and choose **Convert to EML…** to produce a directory tree of standards-compliant `.eml` files mirroring the archive's folder structure. Includes duplicate handling, limits, original SMTP header extraction, and transparent support for Highly Encrypted and Password Protected archives. As with MSG conversion, messages lacking original headers get them synthesized and flagged via `X-MailKit-Synthesized-Headers`. The conversion dialog can also recover messages that are no longer linked into the folder tree — soft-deleted items go to a `Recovered Items` folder and fully detached (orphaned) message nodes to an `Orphaned Items` folder (both on by default). Calendar items — appointments and meeting requests — are exported with an attached calendar invite (`invite.ics`); recurring series keep their full recurrence rule, time zone and deleted occurrences, all-day events stay all-day, and meeting **responses** record the attendee's accept/decline/tentative status in the invite. Delivery-status notifications and read receipts are reconstructed as a proper `multipart/report` (delivery-status / disposition-notification) instead of a flattened body, and S/MIME messages keep their signed/encrypted envelope verifiable (`multipart/signed` / `application/pkcs7-mime`) — the same fidelity as the MSG conversion. Contacts (as vCards), tasks (as VTODO calendars), sticky notes, journal entries and distribution lists (with their resolved members) can be exported too via the opt-in "Convert contacts, tasks, notes and distribution lists" checkbox; most other Outlook item types (documents, recall/remote/resend utility items, item-status reports) now convert as plain messages rather than being skipped, and archives containing item types that are still skipped report a per-class summary in the console. OLE-embedded objects are exported as raw `.ole` attachments instead of being dropped, and an opt-in "Verify on-disk checksums" mode turns silent archive corruption into reported errors. Outlook metadata survives the conversion: `Sensitivity`, conversation threading (`Thread-Topic`/`Thread-Index`), `Reply-To` and attachment `Content-Location` are exported alongside the importance, spam-confidence and journal-report markers.
...
- **Sending EML** — a swaks-equivalent embedded SMTP client. Right-click an `.eml` file in the editor or project view and choose **Send EML…** to push the file's bytes through a configured server. The client's purpose is to deliver your `.eml` exactly as it is on disk: it sends the message **verbatim** and never adds, removes, or rewrites message content. Highlights:
  ...
  - **Envelope address handling** — the envelope `From` / `To` accept display-name forms (`Alice <alice@example.com>`) and comma-separated recipient lists (quoted commas and groups included); MailKit sends the bare address each server expects. The envelope is the *only* thing the dialog supplies; the message body itself is left untouched
  - **Verbatim send** — the message is transmitted byte-for-byte as it appears in the `.eml` file; the client never adds, removes, or rewrites header or body content. Note the trade-off: because the file goes out as-is, a `Bcc:` header present in the `.eml` is transmitted along with it and is therefore visible to recipients — the client does **not** strip `Bcc:` and does not hide blind recipients. Remove any `Bcc:` line from the file yourself if you do not want it disclosed
  - **Normalize line endings (LF→CRLF)** — a checkbox on the Send dialog, **on by default**. When checked (the default), bare-`LF`/`CR` line endings are converted to `CRLF` on the wire as SMTP expects (rfc5321 §2.3.8). Uncheck it to send the body with its original line endings untouched, for true byte-for-byte transmission. Dot-stuffing and the message-terminator framing are required by the protocol and are always applied, so unchecking only suppresses the LF→CRLF conversion
...
8. Select two `.eml` files in the Project view and choose *Compare EML Files* (or right-click a single `.eml`, or an open `.eml` editor, and choose *Compare EML With…*) to open the decoded, normalized diff of the two messages; use the toolbar toggle to switch to the raw source view
9. Configure SMTP profiles in **Settings > Tools > MailKit SMTP**, then right-click any `.eml` file and choose *Send EML…* to push it through the configured server. Wire-level activity streams into the *MailKit* tool window; past sends are catalogued under *Tools > Show Recent SMTP Sends…*
