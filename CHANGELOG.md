...
- A non-Latin message converted from a `.msg` that declared its code page but no separate "internet" code page now decodes its **body** (plain text and HTML) with the correct code page instead of garbling it as Western European — matching how the subject was already handled and how the same message converts from a `.pst`/`.ost`.
- An appointment or meeting converted from `.msg`/`.pst`/`.ost` now keeps its **reminder** ("remind me N minutes before") as a calendar alarm, and its **free/busy status** (Free, Tentative, Out of Office) instead of every event showing as Busy, in the generated `invite.ics`.
- A meeting invitation converted from `.msg`/`.pst`/`.ost` now marks **optional (Cc) attendees** as optional in the generated calendar entry instead of listing everyone as a required participant; required attendees and the organizer are unchanged.
- A message converted from `.msg`/`.pst`/`.ost` now preserves its **follow-up flag** label (e.g. "Follow up", "Review") as an `X-Message-Flag` header, its **expiration time** as an `Expiry-Date` header, and a recorded **voting-button response** (e.g. "Approve") as an `X-MS-Exchange-Vote-Response` header, instead of dropping them.
- An inline image in a converted message whose Content-ID is a leading part of another inline image's Content-ID (for example `image1` and `image10`) no longer has the longer reference corrupted, so both images keep displaying in the body.
- A distribution list converted from a `.pst`/`.ost` no longer drops a member that has a display name but no stored email address — the member is now listed (matching the `.msg` conversion).
- A message attached *inside* a converted `.pst`/`.ost` whose name contains characters that are not valid in a filename (such as `:` or `/`, common in subjects like `Re: Q3 6/30`) now has those characters replaced so the saved `.eml` name is usable — matching the `.msg` conversion.
- An email, appointment, contact, or task attached *inside* a converted `.msg` now keeps all its Outlook-specific fields — an embedded appointment's calendar invite (`invite.ics`), an embedded contact's email and instant-messaging addresses, an embedded task's schedule details, and follow-up flags or categories on an embedded email. Previously every such field was silently dropped for embedded items, because the lookup table that names these fields exists only at the top level of a `.msg` file and the embedded item was read without it.
- A meeting cancellation converted from `.msg`/`.pst`/`.ost` now marks the event as cancelled *inside* the generated `invite.ics` (`STATUS:CANCELLED`), so a calendar program that merely imports the file — without processing the scheduling method — still shows the meeting as cancelled rather than active.
- A message whose only body is rich text (RTF) containing a table no longer welds the table's cells and rows into one unreadable run of text in the extracted plain-text body; cells are now separated by tabs and each row starts on a new line.
- A draft (never-sent) message converted from `.msg`/`.pst`/`.ost` now carries the standard `X-Unsent: 1` marker, so a mail client that honors it reopens the file as an editable draft instead of a read-only received message.
- A contact whose email address is stored as an internal Exchange (X.500) directory path no longer has that raw path written into the exported contact card as if it were an internet email address; it is now encapsulated the same way such addresses already appear in converted message headers. Ordinary internet addresses are unchanged.
- A contact whose instant-messaging address is a full URI with parameters (for example `sip:jane@corp.example;transport=tls`) no longer has that address corrupted by text escaping in the exported contact card; plain IM handles are unchanged.
- An inline image that the HTML body of a converted `.msg` references by its web address (`Content-Location`) rather than a Content-ID is now marked inline, so it no longer shows up both in the body *and* as a separate downloadable attachment — matching the `.pst`/`.ost` conversion.
- A bounce report or read receipt whose human-readable explanation is one very long unwrapped line no longer emits that line over the email line-length limit (which strict mail servers may truncate); such text is now safely encoded and wrapped. Reports with ordinary short or pre-wrapped text are unchanged.
- A sent poll (a message with Outlook voting buttons) converted from `.msg`/`.pst`/`.ost` now keeps its offered choices as an `X-MS-Exchange-Vote-Options` header (e.g. `Approve, Reject`), complementing the already-preserved recorded response.
- A journal entry (`IPM.Activity`) converted with non-mail export enabled now keeps its defining data — the activity type, start and end time, and duration — as `X-Journal-*` headers instead of dropping everything but the subject and notes.
- A sticky note converted with non-mail export enabled now keeps its paper color as an `X-Note-Color` header (Blue, Green, Pink, Yellow, or White).
...
- An HTML message converted from a `.msg`/`.pst`/`.ost` that stored its body only as rich text (HTML-encapsulated RTF) no longer loses a run of body text that immediately followed certain RTF escapes — such as a non-breaking space, optional hyphen, or a literal backslash or brace — and the literal backslash and brace characters those escapes represent are now kept in the recovered HTML instead of being dropped along with the text after them.
- A recurring appointment converted from `.msg`/`.pst`/`.ost` whose stored monthly recurrence is corrupt — an out-of-range day-of-the-month or week-of-the-month value — no longer produces an invalid repeat rule that a strict calendar client rejects outright (silently dropping the entire series); the event is now exported as a single clean occurrence instead.
- An attachment converted from `.msg`/`.pst`/`.ost` whose filename is unusually long no longer emits an over-length header line that violates the email line-length limit and that strict mail clients may reject or truncate; an over-long name is now wrapped across RFC 2231 continuation parameters, while ordinary-length filenames are unchanged.
- A meeting that was originally received from a non-Outlook calendar program and then converted from `.msg`/`.pst`/`.ost` now keeps that meeting's own original identifier as the calendar entry's `UID`, so a later acceptance, update, or cancellation still matches the original meeting in your calendar client instead of appearing as a separate, unrelated event.
- A meeting response converted from a `.pst`/`.ost` no longer names a blind-carbon-copy (Bcc) recipient as the meeting's organizer in the generated calendar entry — matching the `.msg` conversion, which already hid Bcc recipients — so a blind recipient is not disclosed to the other attendees.
- A delivery-status (bounce / non-delivery) report converted from `.msg`/`.pst`/`.ost` that no longer carried the failed recipient's address no longer places a list of display names where an email address belongs in the report's `Final-Recipient` field; it now reports `unknown` there instead, so the report stays valid for mail clients and tools that parse it.
- An HTML message converted from a `.pst`/`.ost` that stored its body only as rich text (HTML-encapsulated RTF) no longer leaves a stray backslash or brace character in the recovered text when a non-Latin character was followed by an alternate plain-text version (its fallback) that itself started with one of those escapes — matching the `.msg` conversion, which already discarded the fallback.
- An HTML message converted from a `.pst`/`.ost` no longer loses the rest of a `<meta>` character-set tag — a trailing parameter such as `; format=flowed`, or any attribute that follows the character set — when the declared character set is rewritten to UTF-8; only the character-set value is changed now, and the remainder of the tag is preserved.
- A read-receipt notification converted from `.msg`/`.pst`/`.ost` now writes the acknowledged message's identifier in angle brackets in the receipt's `Original-Message-ID` field when the stored value lacked them, so strict mail clients accept the receipt and can match it back to the message it confirms.
